...
#to tokenizes and filters the words, and adds them to the training data and labels.
...
#to convert labels into numerical value
num_classes = len(labels)
lbl_encoder = LabelEncoder()#uses ordinal encoding
...
#  to tokenize and pad the training sentences, preparing them for input to the neural network.
...
#to load pre-trained word embeddings (GloVe embeddings) from a file.
embeddings_index = {}
...
model.add(SimpleRNN(16, activation='LeakyReLU',kernel_regularizer=l2(0.01)))
model.add(Dense(16, activation='LeakyReLU',kernel_regularizer=l2(0.01)))
model.add(Dense(16, activation='LeakyReLU',kernel_regularizer=l2(0.01)))
...
def check():
    ...
    while True:
        ...
        if confidence >= confidence_threshold and any(i['tag'] == category for i in data['intents']):
            ...
            print("I am sorry I`m unable to understand")